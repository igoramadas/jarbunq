...
const EmailAction = async (message: any): Promise<any> => {
    ...
    try {
        ...
        if (settings.amazon.refundDays > 0) {
            const targetDate = moment().add(settings.amazon.refundDays, "days")

            const job: ScheduledJob = {
                date: targetDate.toDate(),
                title: `Amazon refund: ${description}`,
                type: "payment",
                options: paymentOptions
            }

            paymentOptions.notes = [`Auto scheduled at ${targetDate.format("ll")}`]
            scheduler.queue(job)

            return `Refund of ${amount.toFixed(2)} scheduled to ${targetDate.format("ll")}`
        } else {
