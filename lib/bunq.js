...
const bunq_js_client_1 = require("@bunq-community/bunq-js-client");
...
class Bunq extends BaseEvents {
    constructor() {
         ...
         * @event makePayment
         */
        this.getAccountBalance = async (alias) => {
            try {
                if (!this.authenticated) {
                    throw new Error("Not authenticated to bunq");
                }
         ...
         */
        this.makePayment = async (options) => {
            const alias = { value: options.toAlias };
            ...
            try {
                ...
                if (refAccount.balance.value - options.amount < settings.bunq.minBalance) {
            ...
            try {
                ...
                if (settings.bunq.disablePayments) {
                    ...
                    if (options.draft) {
                        payment = await bunqClient.api.draftPayment.post(this.user.id, accountId, options.description, {
                            value: options.amount.toString(),
                            currency: options.currency
                        }, alias);
                    }
                    else {
                        payment = await bunqClient.api.payment.post(this.user.id, accountId, options.description, {
                            value: options.amount.toString(),
                            currency: options.currency
                        }, alias);
                    }
         ...
         */
        this.failedPayment = (options, err, step) => {
            let amount = options.amount.toFixed(2);
...
`);
    ...
    async init() {
        ...
        try {
            const store = {
                get: (key) => database.get(`jsClient.${key}`).value(),
                set: (key, value) => database.set(`jsClient.${key}`, value).write(),
                remove: (key) => database.unset(`jsClient.${key}`).write()
            };
            bunqClient = new bunq_js_client_1.default(store);
