    ...
    async init() {
        ...
        if (settings.database.crypto.enabled) {
            if (currentData.substring(0, cryptPrefix.length) != cryptPrefix) {
                logger.warn("Database.init", "database.crypto.enabled = true", "File not encrypted yet, so we will do it now");
            ...
            if (currentData.substring(0, cryptPrefix.length) == cryptPrefix) {
                logger.warn("Database.init", "database.crypto.enabled = false", "File is encrypted, so we will decrypt now");
